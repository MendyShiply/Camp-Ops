(function () {
  ...
  V.requests = function () {
    var requestUrl = location.origin + location.pathname + "?request";
    return "<div class=\"topbar\"><div><h2>Requests</h2><p class=\"muted\">Staff/counselor requests wait here before becoming official tasks.</p></div>" +
      "<button class=\"btn\" id=\"open-request-form\">New request</button></div><section class=\"panel share-panel\"><div><h3>Public request form</h3><p class=\"muted\">Share this link or turn it into a QR code for counselors, kitchen, office, or maintenance requests.</p><code>" + C.esc(requestUrl) + "</code></div><button class=\"btn secondary\" data-view=\"requestForm\">Preview form</button></section><section class=\"panel\">" + V.requestRows(C.state.requests) + "</section>";
  };

  V.requestRows = function (requests) {
    if (!requests.length) return "<div class=\"empty\">No requests yet.</div>";
    return "<div class=\"table-wrap\"><table><thead><tr><th>Request</th><th>Location</th><th>Status</th><th>Actions</th></tr></thead><tbody>" +
      requests.map(function (request) {
        return "<tr><td><strong>" + C.esc(request.title) + "</strong><br><span class=\"muted\">" + C.esc(request.details || "") + "</span></td>" +
          "<td>" + C.esc(C.locationName(request.locationId)) + "</td><td><span class=\"pill warn\">" + C.esc(request.status) + "</span></td><td>" +
