...
alter table public.staff_requests add column if not exists updated_at timestamptz not null default now();
...
alter table public.supply_requests add column if not exists deleted_at timestamptz;
